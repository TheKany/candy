...
현재 흐름: 질문 입력/전송 동의 → 카드 섞기·뽑기 → Gemini 해설.
...
질문 전송 안내는 입력 화면에서 확인합니다. 질문을 바꾸면 다시 동의해야 합니다.
질문은 현재 탭에 보관하며 앱 DB나 서버 로그에는 저장하지 않습니다.
Google 무료 API의 입력/응답은 제품 개선이나 사람이 검토하는 대상이 될 수 있어 개인정보 없는 가상 질문 체험으로 안내합니다.
