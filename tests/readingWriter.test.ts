...
const continuation = { followUpQuestions: ["지금 어떤 행동부터 하면 좋을까요?", "내가 놓친 부분은 무엇일까요?"], contextSummary: "현재 질문의 상황과 카드 해석을 구분해서 기억하는 상담의 핵심 요약이에요." };
...
test("multi-card overview requires two separate explanatory paragraphs", () => {
  ...
  assert.throws(() => parseWrittenReading({ ...reading, ...continuation }, 3));
  const overview = ["지금까지 쌓아온 노력이 어떤 결과로 이어졌는지 살펴보는 것이 이번 질문의 출발점이에요.", "다만 카드만으로 승진을 확정할 수는 없어요. 실제 평가 기준과 기회도 함께 확인해야 해요."];
  assert.deepEqual(parseWrittenReading({ ...reading, ...continuation, overview }, 3).overview, overview);
  assert.throws(() => parseWrittenReading({ ...reading, ...continuation, overview: [overview[0]] }, 3));
});
...
test("rejects truncated prose and missing or extra card pages", () => {
  ...
  assert.equal(parseWrittenReading({ ...reading, ...continuation }, 1).pages.length, 1);
  assert.throws(() => parseWrittenReading({ ...reading, ...continuation }, 3));
  assert.throws(() => parseWrittenReading({ ...reading, ...continuation, pages: [{ ...reading.pages[0], detail: "흐름을 살펴보세요." }] }, 1));
});
